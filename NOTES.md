...
This is the library that I used to ensure some simple object validation and in general it is a good library which helps to maintain code integrity.

As you probably noticed I removed Item-related classes. The reason for that is that the relation between Product and Item (in my opinion) didn't reflect 
the Product and price/units correctly as a whole, it was difficult to understand. The relations between objects were quite weird. 
...
Also, I introduced DiscountType to help working with different types of discounts as they might have different rules etc and it is better to achieve it 
if we encapsulate those different type in different objects.

In Basket class I included discountZero flag to differentiate between different discount but in a real application it might be achieved by any sort of String types, 
...
I've not added many comments in the code as I don't consider it is a good practise as the code should be self-explanatory.
